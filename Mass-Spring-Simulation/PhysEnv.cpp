...
{
	System cur(m_CurrentSys, m_ParticleCnt);
	System temp_end(m_ParticleCnt);

	// Compute the state of the system at the whole interval
	IntegrateSysOverTime(cur, cur, temp_end, DeltaTime);

	// Compute the average derivatives between start and end of interval
	System temp_average = (cur + temp_end) / 2.0f;

	// Compute the state of the system at the whole interval using temp_average
	IntegrateSysOverTime(cur, temp_average, m_TargetSys, DeltaTime);
}
...
void CPhysEnv::RK4Integrate(float DeltaTime)
{
	...
	// Velocity
	System slope = (k1 + k2 * 2.0f + k3 * 2.0f + k4) / 6.0f;

	// Integrate
	IntegrateSysOverTime(m_CurrentSys, slope, m_TargetSys, DeltaTime);
...
void CPhysEnv::RK5Integrate(float DeltaTime)
{
	...
	// Integrate for K2
	//integrate with quarter step to get k2
	IntegrateSysOverTime(m_CurrentSys, k1, k2, DeltaTime / 4.0f);
	ComputeForces(k2);
	// Integrate with quarter for K3
	IntegrateSysOverTime(m_CurrentSys, k1 * 0.5f + k2 * 0.5f, k3, DeltaTime / 4.0f);
	ComputeForces(k3);
	// Integrate for K4
	//integrate with half step to get k4
	IntegrateSysOverTime(m_CurrentSys, k3 * 2.0f - k2, k4, DeltaTime / 2.0f);
	ComputeForces(k4);
	// Integrate for K5
	//integrate with 3/4 step to get k4
	IntegrateSysOverTime(m_CurrentSys, k1 * 0.25f + k4 * 0.75f, k5, DeltaTime * 0.75f);
	ComputeForces(k5);
	// Integrate for K6
	//integrate with full step
	IntegrateSysOverTime(m_CurrentSys, k1 * -3.0f / 7.0f + k2 * 2.0f / 7.0f + k3 * 12.0f / 7.0f - k4 * 12.0f / 7.0f + k5 * 8.0f / 7.0f, k6, DeltaTime);
	ComputeForces(k6);
	// yn+1 = yn + (1/90)(7k1 + 32k3 + 12k4 + 32k5 + 7k6)
	// Velocity
	System slope = (k1 * 7.0f + k3 * 32.0f + k4 * 12.0f + k5 * 32.0f + k6 * 7.0) / 90.0f;
...
void CPhysEnv::RK4AdaptiveIntegrate(float DeltaTime)
{
	...
	IntegrateSysOverTime(m_CurrentSys, k2, k3, halfDeltaT);
	ComputeForces(k3);
	System k_half_step = k3;
	// Integrate for K4
	//integrate from k3 with half step to get k4
	IntegrateSysOverTime(m_CurrentSys, k3, k4, DeltaTime);
	ComputeForces(k4);
	System k_full_step = k4;
	// yn+1 = yn + (1/6)(k1 + 2k2 + 2k3 + k4)
	// Velocity
	System slope = (k1 + k2 * 2.0f + k3 * 2.0f + k4) / 6.0f;
	...
	System k_temp = k_full_step - k_half_step;

	//halved system - full step system gets the error
	current_error = sqrt(pow((k_temp.particles.data()->pos.x), 2) +
						 pow((k_temp.particles.data()->pos.y), 2) +
						 pow((k_temp.particles.data()->pos.z), 2)) /
					15;
	// If the error is high, decrease current step size. And vice versa.
	if (acc >= current_error)
	{
		alpha = 0.2;
	}
	else
	...
	if (current_error == 0.00f)
		{current_error = EPSILON;}

	DeltaTime = DeltaTime * abs(pow((acc / current_error), alpha));

	if (DeltaTime <= 0.009f) DeltaTime = 0.0095f;

	if (DeltaTime >= 0.0101f) DeltaTime = 0.0095f;
...
void CPhysEnv::Simulate(float DeltaTime, BOOL running)
{
	...
	while (CurrentTime < DeltaTime)
	{
		...
		if (running)
		{
			...
			if (m_CollisionRootFinding)
			{
				...
				if (timestamp <= 240.0f)
				{
					...
					{
						out << "RK4AdaptiveIntegrate," << timestamp << "," << i << "," << m_TargetSys[i].pos.x << "," << m_TargetSys[i].pos.y << "," << m_TargetSys[i].pos.z << endl;
					}
				}
			}
			else
			{
				...
				case EULER_INTEGRATOR:
					...
					if (timestamp <= 240.0f)
					{
						for (int i = 0; i < m_ParticleCnt; i++)
						{
							out << "EulerIntegrate," << timestamp << "," << i << "," << m_TargetSys[i].pos.x << "," << m_TargetSys[i].pos.y << "," << m_TargetSys[i].pos.z << endl;
						}
					}
					break;
				case MIDPOINT_INTEGRATOR:
					...
					if (timestamp <= 240.0f)
					{
						for (int i = 0; i < m_ParticleCnt; i++)
						{
							out << "MidPointIntegrate," << timestamp << "," << i << "," << m_TargetSys[i].pos.x << "," << m_TargetSys[i].pos.y << "," << m_TargetSys[i].pos.z << endl;
						}
					}
					break;
				case HEUN_INTEGRATOR:
					...
					if (timestamp <= 240.0f)
					{
						for (int i = 0; i < m_ParticleCnt; i++)
						{
							out << "HeunIntegrate," << timestamp << "," << i << "," << m_TargetSys[i].pos.x << "," << m_TargetSys[i].pos.y << "," << m_TargetSys[i].pos.z << endl;
						}
					}
					break;
				case RK4_INTEGRATOR:
					...
					if (timestamp <= 240.0f)
					{
						for (int i = 0; i < m_ParticleCnt; i++)
						{
							out << "RK4Integrate," << timestamp << "," << i << "," << m_TargetSys[i].pos.x << "," << m_TargetSys[i].pos.y << "," << m_TargetSys[i].pos.z << endl;
						}
					}
					break;
				case RK5_INTEGRATOR:
					...
					if (timestamp <= 240.0f)
					{
						for (int i = 0; i < m_ParticleCnt; i++)
						{
							out << "RK5Integrate," << timestamp << "," << i << "," << m_TargetSys[i].pos.x << "," << m_TargetSys[i].pos.y << "," << m_TargetSys[i].pos.z << endl;
						}
					}
					break;
		...
		if (collisionState == PENETRATING)
		{
			...
			// if (fabs(TargetTime - CurrentTime) > EPSILON)
			// {
			// CurrentTime = TargetTime + EPSILON*.01;
			// }
